...
if __name__ == "__main__":
    ...
    dates = [
        datetime(year=2021, month=9, day=17, hour=11),
        datetime(year=2021, month=8, day=21, hour=11),
        datetime(year=2021, month=10, day=1, hour=11)
    ]

    dates = [i + timedelta(days=2) for i in dates]

    duration = timedelta(days=0, hours=10)
    ...
    tasks = [
        ...
        #"delay-1-li6400xt_PAR_outside_chamber_umol/m2/s",
        #"delay-1000-li6400xt_PAR_outside_chamber_umol/m2/s",
    ...
    for idx_x, data_source in enumerate(data_sources):
        for idx_y, task in enumerate(tasks):
            ...
            if "delay" in task:
                ...
                print(f"Going for {data_source} from {len(df)}")
            ...
            df = df[(df.index >= dates[idx_x]) & (df.index < (dates[idx_x]+duration))]
            ...
            plot_df.to_csv(f"plot_data/time_trace_day_{original_task_to_plot.replace('/', '-')}_{data_source}.csv", index=False)

    plt.show()
    fig_test.savefig(f"plots/delay_plot_day_{original_task_to_plot.replace('/', '-')}_{data_source}.png", dpi=300)